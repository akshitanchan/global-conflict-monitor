"""
Throughput Benchmark Script - With Clean Tables
Compares batch processing time for PostgreSQL aggregation vs Flink incremental updates
"""
...
def measure_postgres_aggregation(batch_size, columns):
    """
    Baseline: INSERT batch + run aggregation queries (simulating full recomputation)
    """
    print(f"\n{'='*70}")
    ...
    aggregations = {
        ...
        'dyad_interactions': """
            SELECT event_date, source_actor, target_actor,
                   ...
                   AVG(goldstein) as avg_goldstein
            FROM gdelt_events 
            WHERE source_actor IS NOT NULL AND target_actor IS NOT NULL
...
def measure_flink_incremental(batch_size, columns):
    """
    Incremental: INSERT batch + wait for Flink CDC to update aggregate tables
    """
    print(f"\n{'='*70}")
...
def print_results_table(results: List[Dict]):
    """
    Print clean, formatted results table
    """
    print("\n" + "="*100)
...
def compare_throughput(batch_sizes=[1000, 5000, 10000]):
    """
    Run comparative throughput benchmark
    """
    print("\n" + "="*70)
