...
with st.sidebar:
    ...
    if st.button("Insert", use_container_width=True):
        ...
        with st.spinner("Running fast append (no truncate)..."):
            try:
                subprocess.run(
                    ["bash", "-lc", f"SMALL_LOAD_LINES={int(ins_lines)} ./scripts/load-gdelt-append.sh {ins_file}"],
                    check=True,
                    capture_output=True,
                    text=True,
                    env=os.environ.copy(),
                ...
                st.error("Insert (fast append) failed.")
        ...
        st.session_state.last_operation = "INSERT_FAST_APPEND"
        st.session_state.last_throughput = (int(ins_lines) / elapsed) if elapsed > 0 else None

        if not got:
            st.warning("Fast append finished, but no NOTIFY arrived within timeout. Aggregations may still be catching up.")
